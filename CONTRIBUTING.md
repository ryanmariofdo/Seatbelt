...
Thanks for considering a contribution to Blueprint — PRs are welcome, including small ones (typo fixes, doc improvements, a single new `Configuration`).

## Getting started

```bash
git clone https://github.com/Hashane/Blueprint.git
cd Blueprint
composer install
```
...
Each boot-time behavior is a small class implementing `Judehashane\Blueprint\Contracts\Configuration`:
...
- Register it in the `configurations` array in `config/blueprint.php`, and add a config key (or nested group, if it needs more than one setting — see `database` or `password` for examples) with a doc comment explaining what it does and why it's gated the way it is.
